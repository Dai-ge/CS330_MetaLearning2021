...
class MANN(nn.Module):
    def __init__(self, num_classes, samples_per_class, hidden_dim):
        ...
        self.K = self.samples_per_class - 1
    ...
    def forward(self, input_images, input_labels):
        ...
        inps_labels[:,:self.K,:,:] = input_labels[:,:self.K,:,:]
        B,_,N,imgSize = input_images.shape
        out1,_ = self.layer1(torch.cat((input_images.reshape(B,-1,imgSize),inps_labels.reshape(B,-1,N)),dim=-1))
        out2,_ = self.layer2(out1)
        return out2.reshape(B,self.K+1,N,N)
        #############################
    ...
    def loss_function(self, preds, labels):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        querySetPreds = preds[:,self.K:,:,:].reshape(-1,self.N)
        querySetLabels = labels[:,self.K:,:,:].reshape(-1,self.N).argmax(1)
        loss = self.criterion(querySetPreds,querySetLabels)
        return loss
        #############################
