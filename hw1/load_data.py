...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        Note:
            ...
            1 query.
        """
        #K+1 = 2 N = 5--> [2,5,784]  [2,5,5]
        #############################
        #### YOUR CODE GOES HERE ####
        K_plus_1,N = self.num_samples_per_class,self.num_classes
        paths = random.sample(self.folders,N)
        labels = np.eye(N)
        #* In Practice, you need to set the shuffle of support set with True, but the hw requires the shullf=False to make the training easier (check hw1 PDF page 2)
        support_labels_imagePaths = get_images(paths,labels,K_plus_1-1,shuffle=False)
        query_labels_imagePaths = get_images(paths,labels,1,shuffle=True)
        images = []
        labels = []
        for label, image_path in support_labels_imagePaths:
            labels.append(label)
            images.append(self.image_file_to_array(image_path,28*28))
        for label, image_path in query_labels_imagePaths:
            labels.append(label)
            images.append(self.image_file_to_array(image_path,28*28))
        
                
        labels = np.asarray(labels,dtype=np.float32).reshape(K_plus_1,N,-1)
        images = np.asarray(images,dtype=np.float32).reshape(K_plus_1,N,-1)
