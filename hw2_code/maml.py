...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        #* post-adaptation support accuracy
        with torch.no_grad():
            preds = self._forward(images,parameters)
            accuracies.append(util.score(preds,labels))
        # ********************************************************
...
def main(args):
    ...
    if log_dir is None:
        log_dir = f'./logs/maml/omniglot_way={args.num_way}_support={args.num_support}_query={args.num_query}_inner_steps={args.num_inner_steps}_inner_lr={args.inner_lr}_learn_inner_lrs={args.learn_inner_lrs}_outer_lr={args.outer_lr}_batch_size={args.batch_size}'  # pylint: disable=line-too-long
