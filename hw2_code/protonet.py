...
class ProtoNet:
    ...
    def __init__(self, learning_rate, log_dir):
        ...
        # self.classificationFunc = lambda x,cn: torch.exp(-self.distanceFunc(x,cn)) / torch.sum(torch.exp(-self.distanceFunc(x,cn)),dim=-1)
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # print(support_shot_nums)
            # print(support_out.shape)
            # print(labels_support.shape) #[5]
            #* cal cn
            cn = support_out.reshape((class_nums,support_shot_nums,-1)).mean(dim=1)
            # print(cn.shape) #
            #* cal out vecs of query set
            query_out = self._network(images_query)
            query_dist = vmap(self.distanceFunc,in_dims=(0,None))(query_out,cn) #* cross entropy will do softmax automatically
            
            #* cal loss
            loss = F.cross_entropy(query_dist,labels_query)
            loss_batch.append(loss)
            
            #* cal support accuracy
            support_dists = vmap(self.distanceFunc,in_dims=(0,None))(support_out,cn) 
...
def main(args):
    ...
    if log_dir is None:
        log_dir = f'./logs/protonet/omniglot_way={args.num_way}_support={args.num_support}_query={args.num_query}_lr={args.learning_rate}_batch_size={args.batch_size}'  # pylint: disable=line-too-long
